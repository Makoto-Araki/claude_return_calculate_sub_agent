...
このうち `NaN`・`Infinity` による `500` の問題は、共通機能 `error-handling` として対応しました(後述)。4301桁以上の `400` は、`error-handling` の仕様でスコープ外と明記しました。`divide` の下限(`0.0`)とdocstringの不足は、この時点では未対応です。
